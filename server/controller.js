...
module.exports = {
    ...
    approveAppointment: (req, res) => {
        ...
        sequelize.query(`update cc_appointments as a set approved = true
    ...
    getPendingAppointments: (req, res) => {
        ...
        where approved = false order by date desc;`)
        .then(dbResult => res.status(200).send(dbResult[0]))
        .catch(err => console.log(err))
    },
    ...
    completeAppointment: (req, res) => {
        ...
        sequelize.query(`update cc_appointments as a set completed = true
        where a.appt_id = ${apptId}`)
            .then(dbResult => res.status(200).send(dbResult[0]))
            .catch(err => console.log(err))
    }
